...
Note: use a venv instance instead of the system-wide installation of Python, so
that you can install these modules without conflicting with the system-installed
version of Python.
...
The schema report takes an exported schema LDIF and creates a CSV file
containing the metadata about each attribute (name, OID, description,
objectclasses, etc).

This report can be used when generating the usage report later. This should be
done once at the beginning of the analysis and then only when the schema
changes.
